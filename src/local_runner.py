...
def main():
    ...
    logger.info("Run check example with default settings")
    ...
    video_translation = manager.extract_and_transcribe(
        VideoTranslation(public_id = public_id,
                        source_file=file), lang=source_language)
    torch.cuda.empty_cache()
    manager = TranslationManager(public_id, api_client, file_repository, device="cuda:1", logger=logger)
    video_translation = manager.translate(video_translation, source_lang=source_language, target_lang=target_language)
    
    manager = TextToSpeechManager(public_id, api_client, file_repository, tts_sample_rate=24000, device="cuda:1", logger=logger)
    video_translation = manager.synthesize(video_translation, source_lang=source_language, target_lang=target_language, voice_conv=True, merge_pipeline="stretch_whole", enhance=True)
