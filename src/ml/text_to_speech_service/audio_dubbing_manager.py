...
sys.path.append("/app/resemble-enhance")
...
class AudioDubbingManager:
    ...
    def enhance_audio(
        self, audio_path, save_path, solver="midpoint", nfe=64, tau=0.5, device="cuda:1"
    ):
        solver = solver.lower()
        nfe = int(nfe)
        lambd = 0.9
        ...
        wav2, new_sr = enhance(
            dwav, sr, device, nfe=nfe, solver=solver, lambd=lambd, tau=tau
        )
