...
class VideoDubbingManager:
    ...
    def get_pause(self, wav_path, sr, seconds=True):
        ...
        speech_timestamps = get_speech_timestamps(
            wav, self.model_vad, return_seconds=seconds
        )

        speech_timestamps[0]["pause"] = 0
        for t_idx in range(1, len(speech_timestamps)):
            speech_timestamps[t_idx]["pause"] = (
                speech_timestamps[t_idx]["start"] - speech_timestamps[t_idx - 1]["end"]
            )
    ...
    def merge_pauses(self, wav_path, sr, timestamps):
        ...
        audio_samples = [
            wav[timestamps[0]["start"] : timestamps[0]["end"] + 1].unsqueeze(0)
        ]
        ...
        self.logger.file_logger.info(
            f"Changed audio length is from {wav.shape[0] / sr} to {audio_final.shape[1] / sr}"
        )
    ...
    def merge_timestamps_speedup(self, df, video_length, source_sample_rate):
        ...
        """
        df["gen_dur"] = df["styled_generated_path"].apply(
            lambda audio_path: self.get_audio_length(audio_path)
        )
        df["pause"] = (
            df["start"].shift(-1) - df["end"]
        )  # пауза между двумя предложениями
        df["pause"] = df["pause"].fillna(0)
        df["dur_gen_pause"] = (
            df["gen_dur"] + df["pause"]
        )  # длина сгенерированной речи + пауза, которую можно сделать
    ...
    def merge_timestamps_pause_based(self, video_translation, vocals_audio):
        ...
        df["gen_dur"] = [
            torchaudio.load(segment.generated_file)[0].shape[1] / 24000
        ...
        df["new_start"] = df.apply(
            lambda x: x.start - x.need_time if x.need_time > 0 else x.start, axis=1
        )
    ...
    def merge_timestamps_stretch_whole(self, video_translation, vocals_audio):
        ...
        df["end"] = [segment.end for segment in video_translation.translated_texts]
        df["texts"] = [
            segment.translation for segment in video_translation.translated_texts
        ]
        ...
        if df.shape[0] > 1:
            ...
            df.loc[df.shape[0] - 1, "pause"] = (
                target_audio_length - df.loc[df.shape[0] - 1, "end"]
            )  # the last pause
        elif df.shape[0] == 1:
            df.loc[0, "pause"] = 0

        audio_first, sr_generated = torchaudio.load(
            video_translation.translated_texts[0].generated_file
        )
        previous_pause = torch.zeros(
            (1, int(video_translation.translated_texts[0].start * sr_generated))
        )
        ...
        for idx, segment in enumerate(video_translation.translated_texts):
            ...
            audio = self.change_pauses(
                segment.generated_file,
                segment.source_file,
                sr_gen=sr_generated,
                sr_source=source_sr,
            )
        ...
        if speed_up_rate > 1:
            audio_generated = time_stretch(
                audio_generated.squeeze().numpy(), sr=sr_generated, rate=speed_up_rate
            )
